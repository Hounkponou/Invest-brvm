...
export default function Sidebar({
  ...
  // Sur mobile, refermer le tiroir après une navigation (meilleure ergonomie tactile).
  const closeOnMobile = () => {
  ...
  return (
    <nav
      ...
      style={{ flexDirection: 'column', width: '260px', backgroundColor: 'var(--bg-panel)', borderRight: '1px solid var(--border-color)', padding: '25px 20px', zIndex: 50, transition: 'transform 0.28s ease' }}
    >
      
      {/* 1.  NOUVEAU LOGO DESIGN */}
      <Link to="/" onClick={closeOnMobile}style={{ textDecoration: 'none', display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '40px' }}>
        <div style={{
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
        ...
        <Link to="/dashboard" onClick={closeOnMobile} className={`nav-btn ${location.pathname === '/dashboard' ? 'active' : ''}`} style={{ width: '100%', textAlign: 'left', padding: '12px 15px', borderRadius: '8px', background: location.pathname === '/dashboard' ? 'var(--accent-blue)' : 'transparent', color: location.pathname === '/dashboard' ? 'white' : 'var(--text-muted)', border: 'none', cursor: 'pointer', fontWeight: 'bold', textDecoration: 'none', display: 'flex', alignItems: 'center', gap: '10px', transition: '0.2s' }}>
          Dashboard
        </Link>
        <Link to="/screener" onClick={closeOnMobile} className={`nav-btn ${location.pathname === '/screener' ? 'active' : ''}`} style={{ width: '100%', textAlign: 'left', padding: '12px 15px', borderRadius: '8px', background: location.pathname === '/screener' ? 'var(--accent-blue)' : 'transparent', color: location.pathname === '/screener' ? 'white' : 'var(--text-muted)', border: 'none', cursor: 'pointer', fontWeight: 'bold', textDecoration: 'none', display: 'flex', alignItems: 'center', gap: '10px', transition: '0.2s' }}>
          Stock Screener
        </Link>
        {/* Accès au nouveau Module Prédictif IA (accent vert = signaux) */}
        <Link to="/predictions" onClick={closeOnMobile} className={`nav-btn ${location.pathname === '/predictions' ? 'active' : ''}`} style={{ width: '100%', textAlign: 'left', padding: '12px 15px', borderRadius: '8px', background: location.pathname === '/predictions' ? 'var(--up-color)' : 'transparent', color: location.pathname === '/predictions' ? 'white' : 'var(--up-color)', border: location.pathname === '/predictions' ? 'none' : '1px solid var(--up-color)', cursor: 'pointer', fontWeight: 'bold', textDecoration: 'none', display: 'flex', alignItems: 'center', gap: '10px', transition: '0.2s' }}>
        ...
        <Link to="/simulator" onClick={closeOnMobile} className={`nav-btn ${location.pathname === '/simulator' ? 'active' : ''}`} style={{ width: '100%', textAlign: 'left', padding: '12px 15px', borderRadius: '8px', background: location.pathname === '/simulator' ? '#9f7aea' : 'transparent', color: location.pathname === '/simulator' ? 'white' : '#9f7aea', border: location.pathname === '/simulator' ? 'none' : '1px solid #9f7aea', cursor: 'pointer', fontWeight: 'bold', textDecoration: 'none', display: 'flex', alignItems: 'center', gap: '10px', transition: '0.2s' }}>
          Backtest Lab
        </Link>
        <Link to="/portfolio" onClick={closeOnMobile} className={`nav-btn ${location.pathname === '/portfolio' ? 'active' : ''}`} style={{ width: '100%', textAlign: 'left', padding: '12px 15px', borderRadius: '8px', background: location.pathname === '/portfolio' ? 'var(--accent-blue)' : 'transparent', color: location.pathname === '/portfolio' ? 'white' : 'var(--text-muted)', border: 'none', cursor: 'pointer', fontWeight: 'bold', textDecoration: 'none', display: 'flex', alignItems: 'center', gap: '10px', transition: '0.2s' }}>
          Portfolio Cloud
        </Link>
      </div>
      ...
      <div style={{ marginTop: 'auto', borderTop: '1px solid var(--border-color)', paddingTop: '20px', display: 'flex', flexDirection: 'column', gap: '15px' }}>
        ...
        <Link to="/" onClick={closeOnMobile}style={{ color: 'var(--text-muted)', textDecoration: 'none', display: 'flex', alignItems: 'center', gap: '8px', fontSize: '0.9em', fontWeight: 'bold', transition: 'color 0.2s' }} onMouseOver={(e) => e.currentTarget.style.color = 'var(--text-main)'} onMouseOut={(e) => e.currentTarget.style.color = 'var(--text-muted)'}>
